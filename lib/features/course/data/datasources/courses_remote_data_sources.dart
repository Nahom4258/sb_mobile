...
import 'package:skill_bridge_mobile/features/course/data/models/video_channels_model.dart';
...
abstract class CourseRemoteDataSource {
  ...
  Future<List<ChapterVideoModel>> fetchCourseVideos(
      String courseId, String channelId);
  Future<bool> updateVideStatus(String videoId, bool isCompleted);
  Future<void> downloadCourseById(String courseId);
  Future<List<VideoChannelsModel>> getVideoChannels({required String courseId});
}
...
class CourseRemoteDataSourceImpl implements CourseRemoteDataSource {
  ...
  Future<List<ChapterVideoModel>> fetchCourseVideos(
    ...
    try {
      ...
      final response = await client.get(
        Uri.parse(
            '$baseUrl/course/video/content/$courseId?channelId=$channelId'),
        headers: {
          'content-Type': 'application/json',
          'authorization': 'Bearer $token'
        },
  ...
  @override
  Future<List<VideoChannelsModel>> getVideoChannels(
    ...
    try {
      ...
      final token = userModelJson['token'];
      final response = await client.get(
        Uri.parse('$baseUrl/videoChannels/courseVideos/$courseId'),
        headers: {
          'content-Type': 'application/json',
          'authorization': 'Bearer $token'
        },
      ...
      if (response.statusCode == 200) {
        ...
        return data.map((e) => VideoChannelsModel.fromJson(e)).toList();
      } else if (response.statusCode == 429) {
        throw RequestOverloadException(errorMessage: 'Too Many Request');
      } else {
        throw ServerException();
