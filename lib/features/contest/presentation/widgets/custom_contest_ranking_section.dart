...
class CustomContestRankingSection extends StatelessWidget {
  const CustomContestRankingSection({
    super.key,
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<FetchCustomContestRankingBloc, FetchCustomContestRankingState>(
      builder: (context, state) {
        if (state is FetchCustomContestRankingFailed) {
          if (state.failure.errorMessage != "No internet connection") {
            return Text(state.failure.errorMessage);
          }
          return NoInternet(
            reloadCallback: () {
              context
                  .read<FetchCustomContestRankingBloc>()
                  .add(FetchCustomContestRankingEvent(customContestId: customContestId));
            },
          );
        ...
        } else if (state is FetchCustomContestRankingLoaded) {
          final rankings = state.contestRank;
                ...
                child: EmptyListWidget(
                  ...
                  reloadCallBack: () {
                    context.read<FetchCustomContestRankingBloc>().add(
                        FetchCustomContestRankingEvent(customContestId: customContestId));
                  },
                ),
              ),
            )
