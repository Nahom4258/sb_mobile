...
class _ContestQuestionsByCategoryPageState
  ...
  void initState() {
    ...
    if (widget.contestQuestionByCategoryPageparams.isCustomContest) {
      for (var contestCategory in widget
          .contestQuestionByCategoryPageparams.customContestCategories) {
        if (!widget.contestQuestionByCategoryPageparams.hasEnded) {
          userChoices[contestCategory.id] = List.generate(
              contestCategory.numberOfQuestions, (index) => 'choice_E');
        }

        if (!widget.contestQuestionByCategoryPageparams.hasEnded &&
            contestCategory.isSubmitted) {
          submittedCategories.add(contestCategory.id);
        }
      }
    } else {
  ...
  void onSubmit() {
    ...
    final customContestQuestionState =
        context.read<FetchCustomContestQuestionsByCategoryBloc>().state;
    if (widget.contestQuestionByCategoryPageparams.isCustomContest) {
      if (customContestQuestionState
          is FetchCustomContestQuestionsByCategoryLoaded) {
        for (int i = 0;
            i < customContestQuestionState.contestQuestions.length;
            i++) {
          userAnswers.add(
            ContestAnswer(
              contestQuestionId:
                  customContestQuestionState.contestQuestions[i].id,
    ...
    final contestQuestionState =
        context.read<FetchContestQuestionsByCategoryBloc>().state;
    if (!widget.contestQuestionByCategoryPageparams.isCustomContest) {
      if (contestQuestionState is FetchContestQuestionsByCategoryLoaded) {
        for (int i = 0; i < contestQuestionState.contestQuestions.length; i++) {
          userAnswers.add(
            ContestAnswer(
              contestQuestionId: contestQuestionState.contestQuestions[i].id,
              userAnswer: categoryAnswers[i],
            ),
          );
        }
    ...
    context.read<ContestSubmitUserAnswerBloc>().add(
          ContestSubmitUserAnswerEvent(
            ...
            isCustomContest:
                widget.contestQuestionByCategoryPageparams.isCustomContest,
          ),
        );
    ...
    if (widget.contestQuestionByCategoryPageparams.isCustomContest) {
      for (int i = 0;
          i <
              widget.contestQuestionByCategoryPageparams.customContestCategories
                  .length;
          i++) {
        if (!submittedCategories.contains(widget
            .contestQuestionByCategoryPageparams
            .customContestCategories[i]
            .id)) {
          changeCategory(widget.contestQuestionByCategoryPageparams
              .customContestCategories[i].id);
  ...
  void updateSubmittedCategories(String categoryId) {
    ...
    if (!widget.contestQuestionByCategoryPageparams.hasEnded) {
      setState(() {
        ...
        if (!widget.contestQuestionByCategoryPageparams.isCustomContest && submittedCategories.length ==
            widget
                .contestQuestionByCategoryPageparams.contestCategories.length) {
           callbackFunction = () {
            context.read<ContestDetailBloc>().add(GetContestdetailEvent(
        ...
        } else if (widget.contestQuestionByCategoryPageparams.isCustomContest && submittedCategories.length ==
            widget
                .contestQuestionByCategoryPageparams.customContestCategories.length) {
          callbackFunction = () {
            context.read<FetchCustomContestDetailBloc>().add(
                FetchCustomContestDetailEvent(
                    customContestId: widget
      ...
      setState(() {
        ...
        if(!widget.contestQuestionByCategoryPageparams.isCustomContest){
          ...
          i++) {
            ...
            final contestCategories = widget.contestQuestionByCategoryPageparams.contestCategories;

            if (i == contestCategories.length - 1) {
              showPopupOnCompletingContest(
                context: context,
                onCompleted: () {
                  if (widget
                      .contestQuestionByCategoryPageparams.isCustomContest) {
                    context.read<FetchCustomContestDetailBloc>().add(
                        FetchCustomContestDetailEvent(
                            customContestId: widget
                                .contestQuestionByCategoryPageparams
        ...
        if(widget.contestQuestionByCategoryPageparams.isCustomContest) {
          for(int i = 0; i < widget.contestQuestionByCategoryPageparams.customContestCategories.length; i++) {
            if (!submittedCategories.contains(widget
                .contestQuestionByCategoryPageparams
                .customContestCategories[i]
                .id)) {
            ...
            final contestCategories = widget.contestQuestionByCategoryPageparams
  ...
  Widget buildWidget(
    ...
    return PopScope(
      ...
      onPopInvoked: (didPop) {},
      child: widget.contestQuestionByCategoryPageparams.isCustomContest
          ? widget.contestQuestionByCategoryPageparams.hasEnded
              ? BlocBuilder<FetchCustomContestAnalysisByCategoryBloc,
                  FetchCustomContestAnalysisByCategoryState>(
                  builder: (_, state) {
                    if (state is FetchCustomContestAnalysisByCategoryFailed) {
                      return Scaffold(
                        body: Center(
                    ...
                    } else if (state is FetchCustomContestAnalysisByCategoryLoading) {
                      return Scaffold(
                        body: QuestionShimmerCard(),
                      );
                    } else if (state is FetchCustomContestAnalysisByCategoryLoaded) {
              ...
              : BlocBuilder<FetchCustomContestQuestionsByCategoryBloc,
                  FetchCustomContestQuestionsByCategoryState>(
                  builder: (_, state) {
                    if (state is FetchCustomContestQuestionsByCategoryFailed) {
                      return Scaffold(
                        body: Center(
                          child: Text(AppLocalizations.of(context)!
                              .error_loading_question),
                        ...
                        is FetchCustomContestQuestionsByCategoryLoading) {
                      return Scaffold(
                        body: QuestionShimmerCard(),
                      );
                    } else if (state
                                    ...
                                    showPopupOnQuitButtonPressed(
                                      ...
                                      onQuitPressed: () {
                                        context
                                            ...
                                            .add(FetchCustomContestDetailEvent(
                                                customContestId: widget
                                                    .contestQuestionByCategoryPageparams
                                                    .contestId));
                                        context
                  ...
                  builder: (_, state) {
                    if (state is FetchContestQuestionsByCategoryFailed) {
                      return Scaffold(
                        ...
                        is FetchContestQuestionsByCategoryLoading) {
                      return Scaffold(
                        body: QuestionShimmerCard(),
                      );
