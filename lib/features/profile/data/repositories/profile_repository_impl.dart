...
class ProfileRepositoryImpl implements ProfileRepositories {
  ...
  Future<Either<Failure, bool>> deleteAccount() async {
    if (await networkInfo.isConnected) {
      try {
        var response = await profileRemoteDataSource.deleteAccount();
  ...
  @override
  Future<Either<Failure, UserRefferalInfoEntity>> getUserRefferalInfo() async {
    if (await networkInfo.isConnected) {
      try {
        final refferalInfo =
            await profileRemoteDataSource.getUserRefferalInfo();
        return Right(refferalInfo);
      } catch (e) {
        return Left(await mapExceptionToFailure(e));
      }
    } else {
  ...
  @override
  Future<Either<Failure, UserCashoutRequestsEntity>>
      getUserCashoutRequests() async {
    if (await networkInfo.isConnected) {
      try {
        final userRequests =
            await profileRemoteDataSource.getUserCashoutRequests();
